        ...
        <>
            <div
                ...
                <div className=' flex flex-col justify-center items-center px-[20px] sm:px-[0px] text-white'>
                    ...
                    {appCtx.paymentData?.ipfsHash ?
                        <CustomButton primary={false} onClick={() => { window.open(`localhost:3000/?ipfsUrl=${appCtx.paymentData?.ipfsUrl}&name=${appCtx.nftData?.name}&description=${appCtx.nftData?.description}&ipfsHash=${appCtx.paymentData?.ipfsHash}&assetType=${appCtx.paymentData?.assetType}&assetId=${appCtx.paymentData?.assetId}&s3url=${appCtx.paymentData?.s3url}`, '_blank') }}
            ...
            {accountAddress ? <>
                <div className='my-[20px] flex justify-around'>
                    ...
                    <WhiteCustomButton onClick={() => { setOption('customize') }} primary={option === 'customize' ? true : false}>Customize Event Page</WhiteCustomButton>
